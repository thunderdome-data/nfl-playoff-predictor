...
function bowlgamesInit() {
	...
	for (var i=0; i<35; i++) {
		...
		for (var j=0; j<activeCalSq.length; j++) {
			...
			if (i == activeCalSq[j]) {
				//console.log("match found " + i + "=" + activeCalSq[j]);
	...
	if (theMonth == 11 && theDate < 16) {
		//console.log("bowls have not started yet");
		tmpDay = 0;
	} else if (theMonth == 11) {
		tmpDay = theDate - 15;
...
function onSqClick(id) {
	//console.log("There are " + dailygames[id] + " on this date");
	//console.log("idarray " + $("#" + id).attr("data-theids"));
...
function loadGSContent(gsData) {
	...
	for (var i=0; i<totalEntries; i++) {
		...
		//console.log(gsData.feed.entry[i].gsx$dateformatted.$t);
		allData[counter] = [ {
		...
		if (i > 0) {
			if (gsData.feed.entry[i].gsx$dateformatted.$t == gsData.feed.entry[i-1].gsx$dateformatted.$t) {
				...
				//console.log("dupe found " + i + " // " + tmpCount);
			} else {
				tmpIndex ++;
				tmpCount = 1;
				dailygames[tmpIndex] = 1;
				//console.log("single found " + i);
			}
		}
		*/
